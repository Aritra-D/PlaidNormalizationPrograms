function displayPSTHSingleElectrode(exampleElectrodeType)
...
elecList = [3 4 7 9 16 21 29 33 40 55 69 74 76 92 101 113 115 123 169 183]; 
end
...
for iElec = 1:length(elecList) %1:size(firingRateData.data,1)
    hPlot = getPlotHandles(numRows,numCols,gridPos,gap); linkaxes(hPlot);

    for cOri2 = 1:5
        for cOri1 = 1:5
            plot(hPlot(cOri2,cOri1),firingRateData.timeVals,squeeze(firingRateData.data(elecList(iElec),1,cOri2,cOri1,:)))
            hold(hPlot(cOri2,cOri1),'on');

            if cOri2 ==1 && cOri1 == 5
                plot(hPlot(cOri2,cOri1),firingRateData.timeVals,(squeeze(firingRateData.data(elecList(iElec),1,1,1,:))+squeeze(firingRateData.data(elecList(iElec),1,5,5,:))),'color',[0.8500, 0.3250, 0.0980],'LineStyle',':','LineWidth',2)
            end
...
%             rescaleData(hPlot,-0.1,0.5,getYLims(hPlot),12);
    ...
    title(hPlot(1,1),['elec: ' num2str(elecList(iElec))]);
    pause;
    hold off;
    clf;
