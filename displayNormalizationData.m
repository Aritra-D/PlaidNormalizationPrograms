...
hSessionPanel = uipanel('Title','Session(s)','titleposition','centertop','fontSize',fontSizeLarge,...
                'Unit','Normalized','Position',[0.05 0.925 0.9 0.08]);
            
hSession = uicontrol('Parent',hSessionPanel,'Unit','Normalized', ...
'BackgroundColor', backgroundColor, 'Position', ...
[0.4 0.83 0.2 0.16], ...
'Style','popup','String',fileNameStringAll,'FontSize',fontSizeLarge);

uicontrol('Parent',hSessionPanel,'Unit','Normalized', ...
    'Position',[0.7 0.1 0.2 0.8], ...
    'Style','pushbutton','String','Select','FontSize',fontSizeMedium, ...
    'Callback',{@selectSession_Callback});

% Draw Session on Figure without creating panel            
% uicontrol('Parent',hFigure,'Unit','Normalized', ...
%     'Position',[0.33 0.975 0.16 0.025],...
%     'Style','text','String','Session(s)','FontSize',fontSizeLarge);
% hSession = uicontrol('Parent',hFigure,'Unit','Normalized', ...
%     'BackgroundColor', backgroundColor, 'Position', ...
%     [0.45 0.975 0.16 0.025], ...
%     'Style','popup','String',fileNameStringAll,'FontSize',fontSizeLarge);
% 
% uicontrol('Parent',hFigure,'Unit','Normalized', ...
%     'Position',[0.62 0.975 0.05 0.028], ...
%     'Style','pushbutton','String','Select','FontSize',fontSizeMedium, ...
%     'Callback',{@selectSession_Callback}); 

    function selectSession_Callback(~,~)
        session = get(hSession,'val'); fileNameStringTMP = fileNameStringListArray{session};
        if strcmp(fileNameStringTMP{1}(1:5),'alpaH');       monkeyName = 'alpaH'; 
        elseif strcmp(fileNameStringTMP{1}(1:7),'kesariH'); monkeyName = 'kesariH';
        end
...
%         [expDates,protocolNames,~,~] = dataInformationPlaidNorm(monkeyName);
        ...
        [ElectrodeStringListAll,ElectrodeListArrayListAll] = getElectrodesList;
        ElectrodeListSession = ElectrodeListArrayListAll{session};
%         numGoodElectrodes = length(allGoodElectrodes);
        ...
        hElectrode = uicontrol('Parent',hParameterPanel,'Unit','Normalized', ...
            'BackgroundColor', backgroundColor, 'Position', ...
            [0.5 1-paramsHeight 0.5 paramsHeight], ...
            'Style','popup','String',ElectrodeStringListAll{session},'FontSize',fontSizeMedium);
        ...
        uicontrol('Parent',hParameterPanel,'Unit','Normalized', ...
            ...
            'Style','text','String','AnalysisType','FontSize',fontSizeMedium);
        hAnalysisType = uicontrol('Parent',hParameterPanel,'Unit','Normalized', ...
            'BackgroundColor', backgroundColor, 'Position', ...
            [0.5 1-2.1*paramsHeight 0.5 paramsHeight], ...
        ...
        signalRange = [-0.1 0.5]; erpPeriod = [0.05 0.2]; fftRange = [0 100];
        ...
        uicontrol('Parent',hTimingPanel,'Unit','Normalized', ...
            ...
            'Style','text','String','Baseline (s)','FontSize',fontSizeSmall);
...
%         end
        ...
        function plotData_Callback(~,~)
            ...
            if isempty(ElectrodeListTMP{1})
                error('No electrode found for analysis! Please try another session!')
            end
            ...
            [psthData,xsFR,firingRates,erpData,timeVals,fftDataBL,fftDataST,freqVals,alphaData,gammaData,ssvepData,RMSvalsERP,~] = getData(folderSourceString,fileNameStringTMP,ElectrodeListTMP,erpRange,blRange,stRange);

            PlotConstant = [4 2 0 -2 -4];
            if analysisType == 7
                error('STA computation method not found') % need to work on STA!
            ...
            elseif analysisType == 2 % computing Firing rate
                    ...
                    for c1 = 1:5
                        for c2=1:5
                            plot(plotHandles(c1+PlotConstant(c1),c2),xsFR,squeeze(psthData(c1,c2,:)),'color',plotColor);
                        end
                    end
                    ...
                    elseif DataSize(1)>1
                        firingRates = squeeze(mean(squeeze(firingRates(:,1,:,:)),1));
                    end
                    firingRatesFlipped = [firingRates(5,:);firingRates(4,:);firingRates(3,:);firingRates(2,:);firingRates(1,:);];
                    ...
                    CRFColors = jet(length(cValsUnique));
                        for iCon = 1:5
                            plot(plotHandles2(1,iCon),cValsUnique,firingRates(iCon,:),'Marker','o','LineWidth',2,'color',CRFColors(iCon,:,:))
                            plot(plotHandles3(1,iCon),cValsUnique2,firingRates(:,iCon),'Marker','o','LineWidth',2,'color',CRFColors(iCon,:,:))
                            plot(hRowCRF(1,1),cValsUnique,firingRates(iCon,:),'Marker','o','LineWidth',2,'color',CRFColors(iCon,:,:))
                            hold(hRowCRF(1,1),'on')
                            plot(hColumnCRF(1,1),cValsUnique2,firingRates(:,iCon),'Marker','o','LineWidth',2,'color',CRFColors(iCon,:,:))
            ...
            elseif analysisType == 3 % computing Raster Plot from spike data
                ...
                if analysisType == 1 % compute ERP
                    ...
                    for c1 = 1:5
                        for c2=1:5
                            plot(plotHandles(c1+PlotConstant(c1),c2),timeVals,squeeze(erpData(c1,c2,:)),'color',plotColor);
                        end
                    end
                    %color Coded Matrix of firing Rate
                    if DataSize(1)==1
                    ...
                    RMSvalsFlipped = [RMSvalsERP(5,:);RMSvalsERP(4,:);RMSvalsERP(3,:);RMSvalsERP(2,:);RMSvalsERP(1,:);];
                    ...
                    % CRF Row-wise & Column-wise
                     CRFColors = jet(length(cValsUnique));
                        for iCon = 1:5
                            plot(plotHandles2(1,iCon),cValsUnique,RMSvalsERP(iCon,:),'Marker','o','LineWidth',2,'color',CRFColors(iCon,:,:))
                            plot(plotHandles3(1,iCon),cValsUnique2,RMSvalsERP(:,iCon),'Marker','o','LineWidth',2,'color',CRFColors(iCon,:,:))
                            plot(hRowCRF(1,1),cValsUnique,RMSvalsERP(iCon,:),'Marker','o','LineWidth',2,'color',CRFColors(iCon,:,:))
                            hold(hRowCRF(1,1),'on')
                            plot(hColumnCRF(1,1),cValsUnique2,RMSvalsERP(:,iCon),'Marker','o','LineWidth',2,'color',CRFColors(iCon,:,:))
                            hold(hColumnCRF(1,1),'on')
                        end
                     hold(hRowCRF(1,1),'off'); hold(hColumnCRF(1,1),'off')
                ...
                elseif analysisType == 4 || analysisType == 5
                        ...
                        for c1 = 1:5
                            for c2=1:5
                                plot(plotHandles(c1+PlotConstant(c1),c2),freqVals,squeeze(fftDataBL(c1,c2,:)),'g');
                                hold(plotHandles(c1+PlotConstant(c1),c2),'on')
%                                 set(plotHandles(c1,c2),'Nextplot','add');
                                plot(plotHandles(c1+PlotConstant(c1),c2),freqVals,squeeze(fftDataST(c1,c2,:)),'k');
%                                 set(plotHandles(c1,c2),'Nextplot','replace');
                        ...
                        elseif DataSize(1)>1
                            alphaData = squeeze(mean(squeeze(mean(alphaData(:,1,:,:,:),5)),1));
                        end
                        alphaDataFlipped = [alphaData(5,:);alphaData(4,:);alphaData(3,:);alphaData(2,:);alphaData(1,:);];
                        ...
                        % CRF Row-wise & Column-wise
                         CRFColors = jet(length(cValsUnique));
                            for iCon = 1:5
                                plot(plotHandles2(1,iCon),cValsUnique,alphaData(iCon,:),'Marker','o','LineWidth',2,'color',CRFColors(iCon,:,:))
                                plot(plotHandles3(1,iCon),cValsUnique2,alphaData(:,iCon),'Marker','o','LineWidth',2,'color',CRFColors(iCon,:,:))
                                plot(hRowCRF(1,1),cValsUnique,alphaData(iCon,:),'Marker','o','LineWidth',2,'color',CRFColors(iCon,:,:))
                                hold(hRowCRF(1,1),'on')
                                plot(hColumnCRF(1,1),cValsUnique2,alphaData(:,iCon),'Marker','o','LineWidth',2,'color',CRFColors(iCon,:,:))
                                hold(hColumnCRF(1,1),'on')
                            end
                         hold(hRowCRF(1,1),'off'); hold(hColumnCRF(1,1),'off')
                        ...
                        gammaDataFlipped = [gammaData(5,:);gammaData(4,:);gammaData(3,:);gammaData(2,:);gammaData(1,:);];
                        ...
                        % CRF Row-wise & Column-wise
                         CRFColors = jet(length(cValsUnique));
                            for iCon = 1:5
                                plot(plotHandles2(1,iCon),cValsUnique,gammaData(iCon,:),'Marker','o','LineWidth',2,'color',CRFColors(iCon,:,:))
                                plot(plotHandles3(1,iCon),cValsUnique2,gammaData(:,iCon),'Marker','o','LineWidth',2,'color',CRFColors(iCon,:,:))
                                plot(hRowCRF(1,1),cValsUnique,gammaData(iCon,:),'Marker','o','LineWidth',2,'color',CRFColors(iCon,:,:))
                                hold(hRowCRF(1,1),'on')
                                plot(hColumnCRF(1,1),cValsUnique2,gammaData(:,iCon),'Marker','o','LineWidth',2,'color',CRFColors(iCon,:,:))
                                hold(hColumnCRF(1,1),'on')
                            end
                         hold(hRowCRF(1,1),'off'); hold(hColumnCRF(1,1),'off')                            
...
%                                 set(plotHandles(c1,c2),'Nextplot','replace');
                        ...
                        ssvepDataFlipped = [ssvepData(5,:);ssvepData(4,:);ssvepData(3,:);ssvepData(2,:);ssvepData(1,:);];
                        ...
                        % CRF Row-wise & Column-wise
                         CRFColors = jet(length(cValsUnique));
                            for iCon = 1:5
                                plot(plotHandles2(1,iCon),cValsUnique,ssvepData(iCon,:),'Marker','o','LineWidth',2,'color',CRFColors(iCon,:,:))
                                plot(plotHandles3(1,iCon),cValsUnique2,ssvepData(:,iCon),'Marker','o','LineWidth',2,'color',CRFColors(iCon,:,:))
                                plot(hRowCRF(1,1),cValsUnique,ssvepData(iCon,:),'Marker','o','LineWidth',2,'color',CRFColors(iCon,:,:))
                                hold(hRowCRF(1,1),'on')
                                plot(hColumnCRF(1,1),cValsUnique2,ssvepData(:,iCon),'Marker','o','LineWidth',2,'color',CRFColors(iCon,:,:))
                                hold(hColumnCRF(1,1),'on')
                            end
                         hold(hRowCRF(1,1),'off'); hold(hColumnCRF(1,1),'off')
...
function [psthData,xsFR,firingRates,erpData,timeVals,fftDataBL,fftDataST,freqVals,alphaData,gammaData,ssvepData,RMSvalsERP,electrodeArray] = getData(folderSourceString,fileNameStringTMP,ElectrodeListTMP,erpRange,blRange,stRange)
            if length(fileNameStringTMP) ==1
                [psthData,xsFR,firingRates,erpData,timeVals,fftDataBL,fftDataST,freqVals,alphaData,gammaData,ssvepData,RMSvalsERP,electrodeArray] = getDataSingleSession(folderSourceString,fileNameStringTMP,ElectrodeListTMP,erpRange,blRange,stRange); % First session

            elseif length(fileNameStringTMP)>1
                numDatasets = length(fileNameStringTMP);

                disp(['Working on dataset 1 of ' num2str(numDatasets)]);
                [psthData,xsFR,firingRates,erpData,timeVals,fftDataBL,fftDataST,freqVals,alphaData,gammaData,ssvepData,RMSvalsERP,electrodeArray] = getDataSingleSession(folderSourceString,fileNameStringTMP,ElectrodeListTMP{1},erpRange,blRange,stRange); % First session
            end

% [psthData,xsFR,firingRates,erpData,timeVals,fftData,freqVals,alphaData,ssvepData,electrodeArray,perCorrect,uniqueOrientationChangeDeg] = getDataSingleSession(folderSourceString,fileNameStringTMP{1}); % First session
...
if length(fileNameStringTMP)>1
        for i=2:numDatasets
            ...
            [psthDataTMP,xsFR,firingRatesTMP,erpDataTMP,timeVals,fftDataBLTMP,fftDataSTTMP,freqVals,alphaDataTMP,gammaDataTMP,ssvepDataTMP,RMSvalsERPTMP,~] = getDataSingleSession(folderSourceString,fileNameStringTMP,ElectrodeListTMP{i},erpRange,blRange,stRange);
...
function [psthData,xsFR,firingRates,erpData,timeVals,fftDataBL,fftDataST,freqVals,alphaData,gammaData,ssvepData,RMSvalsERP,electrodeArray] = getDataSingleSession(folderSourceString,fileNameStringTMP,ElectrodeListTMP,erpRange,blRange,stRange)

% folderSaveString = fullfile(folderSourceString,'Projects','PlaidNormalizationProject');
% folderSave = fullfile(folderSaveString,'savedData');
...
% Get Combinations
[parameterCombinations,parameterCombinations2,aValsUnique,eValsUnique,~,~,~,cValsUnique,tValsUnique, ...
                                              aValsUnique2,eValsUnique2,~,~,~,cValsUnique2,tValsUnique2] = loadParameterCombinations(folderExtract);
...
%     Get bad trials
    ...
    %                    lfpDataCon{c1,c2} = analogData(goodPos,:);  %#ok<NODEF>
                       ...
                       firingRates(iElec,t,c1,c2) =  mean(getSpikeCounts(spikeData(goodPos),stRange))/diff(stRange);
...
% Accessory Functions
function [parameterCombinations,parameterCombinations2,aValsUnique,eValsUnique,sValsUnique,fValsUnique,oValsUnique,cValsUnique,tValsUnique,aValsUnique2,eValsUnique2,sValsUnique2,fValsUnique2,oValsUnique2,cValsUnique2,tValsUnique2] = loadParameterCombinations(folderExtract)
...
function [ElectrodeStringListAll,ElectrodeArrayListAll] = getElectrodesList

[tmpElectrodeStringList,tmpElectrodeArrayList,allElecs,monkeyNameList] = getGoodElectrodesDetails;
