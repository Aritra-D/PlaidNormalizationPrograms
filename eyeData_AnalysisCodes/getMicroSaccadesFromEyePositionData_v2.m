...
function [MS,MSLength,numMSInRange,eyeSpeedX,eyeSpeedY,cutoffX,cutoffY,eyeSpeedMag] = getMicroSaccadesFromEyePositionData_v2(monkeyName,folderSourceString,timeRange,threshold,minCutOff,minMSLength)

% get eyeData for an individual Monkey 
if ~exist('folderSourceString','var');     folderSourceString='E:';             end
folderEyeData = fullfile(folderSourceString,'Projects\Aritra_PlaidNormalizationProject\savedData_Figures\EyeData');
fileName = fullfile(folderEyeData,['EyeData_' monkeyName '.mat']);
if exist(fileName,'file')
    load(fileName,'eyeDataDegX','eyeDataDegY','eyeData','timeValsEyeData','trialNums','FsEye');
else
    cleanDataFolder = fullfile(folderSourceString,'data\PlaidNorm');
    [eyeDataDegX,eyeDataDegY,eyeData,timeValsEyeData,trialNums,FsEye] = getEyeDataIndividualMonkey(monkeyName,cleanDataFolder);
end
    
if ~exist('timeRange','var');     timeRange=[timeValsEyeData(1) timeValsEyeData(end)];               end
if ~exist('threshold','var');     threshold  = 3;               end
